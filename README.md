...
More information about the archiving project can be found on the ArchiveTeam wiki: [History Hub](https://wiki.archiveteam.org/index.php?title=History Hub)
...
after which the project container can be run:
    ...
    docker run -d --name archiveteam --label=com.centurylinklabs.watchtower.enable=true --log-driver json-file --log-opt max-size=50m --restart=unless-stopped atdr.meo.ws/archiveteam/historyhub-grab --concurrent 1 YOURNICKHERE
