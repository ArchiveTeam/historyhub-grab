...
local item_patterns = {
  ["^https?://historyhub%.history%.gov/f/discussions/([0-9]+)/dummy"] = "discussion",
  ["^https?://(historyhub%.history%.gov/cfs.+)$"] = "asset",
  ["^https?://(historyhub%.history%.gov/tinymce.+)$"] = "asset",
  ["^https?://(historyhub%.history%.gov/[^/]*image[^/]*/.+)"] = "asset"
}
...
allowed = function(url, parenturl)
  ...
  if string.match(url, "[{}<>]")
    or string.match(url, "[%?&]ReplySortBy=")
    or string.match(url, "^https?://[^/]+/login[%?/]") then
    return false
  end

  if string.match(url, "/utility/scripted%-file%.ashx")
    and (
      (
        not string.match(url, "&_cf=style.vm&")
        and not string.match(url, "&_cf=ui%.js%.vm&")
        and not string.match(url, "&_cf=callback%-model%-list%-replies%.vm&")
        and not string.match(url, "&_cf=content%.vm&")
      )
      or (
        parenturl
  ...
  if not string.match(url, "^https?://historyhub%.history%.gov/") then
  ...
  for _, pattern in pairs({
    ...
    for s in string.gmatch(urlparse.unescape(url), pattern) do
...
wget.callbacks.get_urls = function(file, url, is_css, iri)
  ...
  local function check(newurl)
    ...
    if not processed(url_)
      ...
      local headers = {}
      if string.match(url, "/utility/scripted%-file%.ashx") then
  ...
  local function queue_pages(children, url, parent_id)
    local count = get_count(children)
    if count >= 10 then
      local last = children[count]
      check(set_new_params(url, {
        ["_w_parentId"]=parent_id,
        ["_w_startReplyId"]=last["id"],
        ["_w_initial"]="false",
        ["_w_includeSiblings"]="false",
        ["_w_flattenedDepth"]="0",
        ["_w_replyId"]=""
      }))
    end
    for _, child in pairs(children) do
      if child["children"] then
        queue_pages(child["children"], url, child["id"])
      end
    end
  end
  ...
  if allowed(url)
    ...
    if string.match(url, "/f/[^/]+/[0-9]+/[^/%?]+/?$")
      ...
      for key, default in pairs({
        ...
        local value = string.match(replies_data, "[^0-9a-zA-Z]" .. key .. "%s*:%s*['\"]?([0-9a-zA-Z%-_%.]+)['\"]?")
        if value == "null" then
          value = nil
        end
        ...
        newurl = newurl .. "&_w_" .. key .. "=" .. value
      end
      check(newurl)
    end
    if string.match(url, "&_cf=callback%-model%-list%-replies%.vm&")
      and not string.match(url, "&_w_replyId=[0-9]") then
      json = cjson.decode(html)
      queue_pages(json["replies"], url)
    end
    if json then
...
wget.callbacks.finish = function(start_time, end_time, wall_time, numurls, total_downloaded_bytes, total_download_time)
  ...
  for key, data in pairs({
    ["historyhub-5j40z3xpmr5r5003"] = discovered_items,
    ["urls-b1msnkgosf12y1pd"] = discovered_outlinks
  }) do
